...
7. 完整继承 `material_bank.source_trace.claims`；不得修改既有 Claim 的状态、引用和锁定状态。
8. 新增创作内容必须追加为 `status=PROPOSAL`、`locked=false` 的 Claim，等待当前故事种子由用户确认后再由 Runtime 记录授权关系。
...
- 不把多个热门元素无逻辑地堆进同一个故事。
- 不把原创补全伪装成用户素材。
- 不把 `PROPOSAL` 或 `INFERENCE` 写入已锁定事实。
